...
if [ -z "$TRAVIS_TAG" ]; then
    # Default to using changelog version unless explicitly disabled
    if [ "${USE_CHANGELOG_VERSION:-1}" = "1" ]; then
        # Prefer the first version in debian/changelog
        if command -v dpkg-parsechangelog >/dev/null 2>&1; then
            RELEASE_DEBIAN=$(dpkg-parsechangelog -S Version)
            echo "Using version from debian/changelog: ${RELEASE_DEBIAN}"
        else
            eval "$(egrep -o 'APPVERSION.*=.*' lib/PACUtils.pm | tr -d '[:space:]')"
            RELEASE_DEBIAN=$APPVERSION~$(git log -1 | grep -i "^commit" | awk '{print $2}');
            echo "dpkg-parsechangelog not found. Falling back to git hash version: ${RELEASE_DEBIAN}"
        fi
    else
        eval "$(egrep -o 'APPVERSION.*=.*' lib/PACUtils.pm | tr -d '[:space:]')"
        RELEASE_DEBIAN=$APPVERSION~$(git log -1 | grep -i "^commit" | awk '{print $2}');
...
# Only auto-increment changelog when not explicitly using existing changelog version
if [ "${USE_CHANGELOG_VERSION:-1}" != "1" ]; then
    if ! [[ -z "$TRAVIS_TAG" ]]; then
        dch -v "$DEBIAN_VERSION" -D "unstable" -b -m "New automatic GitHub build from snapshot"
    else
        # Development snapshot builds default to unstable, not stable
        dch -v "$DEBIAN_VERSION" -D "unstable" -b -m "New automatic GitHub build from git hash"
    fi
fi
